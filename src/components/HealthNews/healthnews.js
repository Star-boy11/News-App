...
function HealthNews() {
  ...
  useEffect(() => {
    async function GetHealthData(page) {
      try {
        let url =
          `https://newsapi.org/v2/top-headlines?country=in&category=health&apiKey=5f6a53bf1d3742a5a0a2a0693f1ef917&page=${page}`;

        let data = await fetch(url);
        let hdata = await data.json();
        setHealthData(hdata.articles);
        setTotalResults(hdata.totalResults);
        console.log(hdata)
      } catch (error) {
        console.log("Error fetching data: ", error);
      }
    }
    GetHealthData(currentPage);
  }, [currentPage]);
  ...
  return (
    <div className="main p-2">
      <Container fluid>
        <h2>IndiaNews.com - Healthgtrgetr</h2>
        {healthData.length === 0 ? (
          <p>Loading...</p>
        ) : (
          <Row xs={1} md={2} lg={3} xxl={4} className="g-4 mt-2">
            {healthData.map((article, idx) => (
              <Col key={idx}>
