...
function BusinessNews() {
  ...
  useEffect(() => {
    async function fetchBusinessNews(page) {
      let url = `https://newsapi.org/v2/top-headlines?country=in&category=business&apiKey=5f6a53bf1d3742a5a0a2a0693f1ef917&page=${page}`;

      try {
        let data = await fetch(url);
        let parsedData = await data.json();
        setBusinessData(parsedData.articles);
  ...
  return (
    <div className="main p-2">
      <Container fluid>
        <h2>IndiaNews.com - Business</h2>
        <Row xs={1} md={2} lg={3} xxl={4} className="g-4 mt-2">
          {businessData.map((article, idx) => (
            <Col key={idx}>
