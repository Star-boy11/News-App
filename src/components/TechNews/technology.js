...
function TechNews() {
  ...
  useEffect(() => {
    async function GetTechData(page) {
      try {
        let url =
          `https://newsapi.org/v2/top-headlines?country=in&category=technology&apiKey=5f6a53bf1d3742a5a0a2a0693f1ef917&page=${page}`;

        let data = await fetch(url);
        let tech_data = await data.json();
        setTechData(tech_data.articles);
        setTotalResults(tech_data.totalResults)
        console.log(tech_data)
      } catch (error) {
        console.log("Error fetching data: ", error);
      }
    }
    GetTechData(currentPage);
  }, [currentPage]);
  ...
  return (
    ...
    <Container fluid>
      <h2>IndiaNews.com - Technology</h2>
      <Row xs={1} md={2} lg={3} xxl={4} className="g-4 mt-2">
        {techData.map((article, idx) => (
          <Col key={idx}>
